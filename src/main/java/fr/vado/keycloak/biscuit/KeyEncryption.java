...
/**
 * Chiffrement optionnel de la seed persistée : AES-256-GCM, format {@code enc:v1:<base64(iv||ciphertext)>}.
...
final class KeyEncryption {
    ...
    static String encrypt(byte[] plaintext, byte[] kek) {
        try {
    ...
    static byte[] decrypt(String stored, byte[] kek) {
        try {
